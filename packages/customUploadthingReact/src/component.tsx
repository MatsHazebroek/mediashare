...
export function UploadButton<TRouter extends void | FileRouter = void>(props: {
  ...
  files?: (numberOfFiles: number) => void;
  disabled?: boolean;
  children: (
    isUploading: boolean,
    permittedFileTypes: {
  ...
  useEffect(() => {
    ...
    if (props.files) props.files(file?.length ?? 0);
  }, [startUpload, file, fileInputRef]);
  const upload = useCallback(() => {
    if (!file) return;
    startUpload(file);
    if (fileInputRef.current) fileInputRef.current.value = "";
  }, [file, startUpload, fileInputRef]);
  ...
  return (
    <>
      ...
      >
        {props.children(isUploading, { fileTypes, multiple })}
      </div>
    </>
  );
}
